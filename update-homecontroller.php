...
// Read the current HomeController
$homeControllerPath = '/var/www/casino-portal/src/Controllers/HomeController.php';
$content = file_get_contents($homeControllerPath);

// Find the start and end of the getTopCasinos method
$startPattern = 'private function getTopCasinos(): array {';
$endPattern = '        ];
    }
...
if ($startPos === false || $endPos === false) {
    echo "Could not find the getTopCasinos method boundaries\n";
    exit(1);
}

// New method content
$newMethod = '    private function getTopCasinos(): array {
...
// Replace the content
$newContent = substr($content, 0, $startPos) . $newMethod . substr($content, $endPos + strlen('        ];
    }
...
echo "Successfully updated HomeController to use CasinoDataService\n";
?>
